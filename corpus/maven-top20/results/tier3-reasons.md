# Tier-3 reasons (import ERRORs and jk failures), grouped — run10

Generated 2026-09-17 07:20. Module prefixes, coordinates, versions and paths are normalized so one line = one distinct cause = one ticket candidate.
...
- (1) Resolving dependency graph...  
  repos: keycloak
- (1) confluent is https://packages.confluent.io/maven in G:A and https://packages.confluent.io/maven/ in G:A; a workspace resolves against o  
  repos: quarkus

## jk build failure

- (1) <path> error: cannot find symbol · symbol:   class LabelExpressionLexer · location: package hudson.model.labels  
  repos: jenkins
- (1) <path> error: package org.neo4j.shaded.lucene9.index does not exist  
  repos: neo4j
- (1) exporting a package from system module jdk.javadoc is not allowed with --release  
  repos: hadoop
- (1) no launchable `main` method found in <path> — set `[application] main = "..."` in jk.toml  
  repos: nacos
- (1) protoc failed (exit 1): · tbmsg.proto: File not found. · G:A: Import "tbmsg.proto" was not found or had errors.  
  repos: thingsboard
- (1) quarkus-augment failed (exit 1): · jk-quarkus-augment: locked runtime closure=226 pure-bootstrap · SLF4J(W): No SLF4J providers were found.  
  repos: floci

## jk test failure

- (1) + [1 of 8] G:A - took 22ms  
  repos: dataease
- (1) Failure  
  repos: zipkin
...
- (1) test failure: com.ctrip.framework.apollo.build.sql.converter.ApolloSqlConverterAutoGeneratedTest#checkAutoGenerated() — java.lang.IllegalStateException: illegal class path: <path>  
  repos: apollo
